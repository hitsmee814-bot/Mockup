...
import { HotelCompactSearchBar } from "./compact-search-bar"
...
export function HotelBooking() {
  ...
  const showResults = searchParams.has("destination")

  const handleSearch = (params: { destination: string; checkIn: string; checkOut: string; rooms: number; adults: number; children: number }) => {
                ...
                <motion.div animate={{ y: [0, -12, 0] }} transition={{ duration: 1.5, repeat: Infinity, ease: "easeInOut" }}>
                  <div className="relative">
                    <motion.div
                      ...
                      style={{ background: "#3FB8FF30" }}
                      animate={{ scale: [1, 1.4, 1] }}
                      transition={{ duration: 1.5, repeat: Infinity }}
                    />
                    <Hotel className="h-10 w-10 relative" style={{ color: "#3FB8FF" }} />
                  </div>
                </motion.div>
                <div className="text-center">
                  <p className="text-lg font-semibold text-gray-800">Searching hotels...</p>
                  <p className="text-sm text-gray-400 mt-1">Finding the best stays for you</p>
                </div>
          ...
          {!showResults ? (
            ...
            >
              <div className="text-center space-y-4 pt-6">
                ...
                >
                  <Sparkles className="size-3.5" /> Premium Hotel Booking
                </motion.div>
                <motion.h1
                ...
                >
                  Stay Anywhere,{" "}
                  <span style={{ color: "#3FB8FF" }}>In Style</span>
                </motion.h1>
                <motion.p
                  className="text-gray-400 text-sm sm:text-base max-w-lg mx-auto leading-relaxed"
                  initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
                >
                  Discover handpicked hotels, resorts & boutique stays — from budget-friendly to ultra-luxury, all at the best prices.
                </motion.p>
              </div>
            ...
            <motion.div
              ...
              className="space-y-4"
            >
              <HotelCompactSearchBar onSearch={handleSearch} />
              <HotelResults />
            </motion.div>
          )}
        </AnimatePresence>
